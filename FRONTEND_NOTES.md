...
| PoC file | Role |
|---|---|
| `live-app/assets/app.js` | SPA framework: hash router, `PB.state`, brand selector, header, toasts |
| `live-app/assets/api.js` | `window.PBApi`, thin client over the local proxy |
| `live-app/proxy_server.py` | Static server + `/api/*` proxy that injects `X-API-Key`, 300s GET cache |
...
`proxy_server.py` forwards `/api/*` → `https://www.aipeekaboo.com/api/v1/*`, injects `X-API-Key`, and caches GETs in memory for 300s. Exact same shape as a catch-all route handler:
...
Source: `live-app/views/todos.js`. Two pure functions, exported as `window.PBTodosLogic` and covered by `live-app/tests/todos.logic.test.mjs` (16 tests, zero deps):

- `normalizeSnapshot(snap, promptRows, brandName, brandUrl)` → normalized shape (spec section 2)
- `aimGenerateTodos(snapNorm)` → `Recommendation[]` (~1,100 lines, 16 rule blocks, spec sections 3 and 4). No DOM, no network, deterministic, sub-second.
...
- Port `normalizeSnapshot` + `aimGenerateTodos` (rename `generateRecommendations`) into `lib/recommendations/engine.ts` as a plain TS module, no React imports. Port the helpers verbatim: `cleanDomain`, `modelIdToProv`, `splitMentions`, `_aimExpandTodo` (parent/suggestion expansion with the singleton-fold rule). Do not change rule semantics; translate the 16 node tests into Vitest and keep them green before touching anything.
- Type the output against the `Recommendation` schema in spec section 3. While porting, fix limitation 6 from the spec: make `effort` an enum `'low' | 'medium' | 'high'` and format the "Low effort" label in the UI. (The PoC's Medium effort filter is broken because it substring-matches `"medium"` against `"Med effort"`.)
- Cache the generated array in PlanetScale via Prisma keyed `(brandId, timeRange)`, invalidated when a new analysis lands (or simply with a TTL): the output only changes when an analysis runs.
- Server-side is also where the missing inputs live: per-model visibility/sentiment/position (`latest_by_provider`) and the competitor heatmap are empty in the public snapshot, which is why rules 4, 10, and 14 never fire in the PoC. Feeding the real internal data into `normalizeSnapshot` upgrades the engine with zero rule changes.
...
Port the same `lib/recommendations/engine.ts` module, but call it from a `useRecommendations(brandId, range)` hook that runs the two API calls through `/api/peekaboo/*`. Ships faster, identical UI code; downsides: logic visible in the bundle, no shared caching, per-model data still missing, and you still need a persistence endpoint for triage state anyway. If you start with B, the move to A later is only relocating one import.
...
- The PoC's per-call `.catch(() => null)` guards and the `td-no-data` fallback todo are behavior, not incidental: a failed endpoint must degrade to the fallback row, never an empty/broken table.
...
1. `lib/recommendations/engine.ts`: port `normalizeSnapshot`, `aimGenerateTodos`, helpers; translate `tests/todos.logic.test.mjs` to Vitest; green before any UI work. Fix the effort enum while porting (the only sanctioned semantic change).
2. `app/api/peekaboo/[...path]/route.ts` + `PEEKABOO_API_KEY` env; `lib/peekaboo.ts` typed client.
3. `stores/dashboard.ts` (persisted brand/model/range) + header components (brand switcher, model select, range picker) on Radix.
4. `app/api/brands/[id]/recommendations/route.ts` + Prisma `TodoItem` + todo-state endpoints.
5. `app/todos/page.tsx` + client tree: Tabs, filter Selects, table, detail panel, bulk bar, notes form. cva badges, Lucide imports, sonner toasts.
6. Verify in the browser against the PoC side by side (`python3 live-app/proxy_server.py`, localhost:7898, `#/todos`) with the same brand: row counts, rule ids, and signal text should match exactly for identical inputs.
